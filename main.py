...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    conv_1x1_upsampled_layer7 = tf.layers.conv2d_transpose(conv_1x1_layer7, num_classes, kernel_size=4, strides=(2, 2),
                  padding='same',kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    ##layer4
    conv_1x1_layer4 = tf.layers.conv2d(vgg_layer4_out, num_classes, kernel_size=1, strides=(1, 1), padding="same",
                                kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    ##skip layer
    layer4_skip = tf.add(conv_1x1_upsampled_layer7, conv_1x1_layer4)
    layer4_upsampled = tf.layers.conv2d_transpose(layer4_skip, num_classes, kernel_size=4, strides=(2, 2),padding="same",
                                kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    #layer 3
    conv_1x1_layer3 = tf.layers.conv2d(vgg_layer3_out, num_classes, 1, padding="same",
                                kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    ## skip layer
    layer3_skip = tf.add(layer4_upsampled, conv_1x1_layer3)
    layer3_upsampled = tf.layers.conv2d_transpose(layer3_skip, num_classes, kernel_size=16, strides=(8, 8),
                                                  padding="same",
                                                  kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    ##chnaged
