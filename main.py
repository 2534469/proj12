...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    conv_1x1_layer7 = tf.layers.conv2d(vgg_layer7_out, filters=num_classes, kernel_size=1, strides=1,
                                       padding='same',
                                       kernel_initializer=kernel_initializer,
                                       kernel_regularizer=tf.contrib.layers.l2_regularizer(l2reg),
                                       name='conv_1x1_layer7')
    conv_1x1_upsampled_layer7 = tf.layers.conv2d_transpose(conv_1x1_layer7,
                                                           filters=num_classes,
                                                           kernel_size=4,
                                                           strides=2,
                                                           padding='same',
    ...
    ##skip layer
    layer4_skip = tf.add(conv_1x1_upsampled_layer7, conv_1x1_layer4, name='layer4_skip')

    layer4_upsampled = tf.layers.conv2d_transpose(layer4_skip, filters=num_classes,
    ...
    # layer 3
    conv_1x1_layer3 = tf.layers.conv2d(vgg_layer3_out, filters=num_classes,
                                       kernel_size=1,
                                       strides=1,
                                       padding='same',
                                       kernel_initializer=kernel_initializer,
    ...
    ## skip layer
    layer3_skip = tf.add(layer4_upsampled, conv_1x1_layer3, name='layer3_skip')
    layer3_upsampled = tf.layers.conv2d_transpose(layer3_skip, filters=num_classes,
                                                  kernel_size=16, strides=8,
                                                  padding='same',
                                                  kernel_initializer=kernel_initializer,
                                                  kernel_regularizer=tf.contrib.layers.l2_regularizer(l2reg),
                                                  name='layer3_upsampled')
    return layer3_upsampled
...
def optimize(nn_last_layer, correct_label, learning_rate, num_classes):
    ...
    total_loss = tf.add(cross_entropy_loss, regularization_loss * regularization_constant, name='total_loss')
...
def run():
    ...
    with tf.Session() as sess:
        ...
        last_layer = layers(layer_3, layer_4, layer_7, num_classes)
